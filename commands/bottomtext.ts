...
module.exports = {
	...
	async run(message:any, commandArgs:any, lastImage:any) {
        ...
        ctx.lineWidth = 8;
        ctx.strokeText(commandArgs.toLocaleUpperCase(), canvas.width / 2.9 - textWidth / 2.9, canvas.height * 0.95);
        ctx.fillStyle = 'white';
        ctx.fillText(commandArgs.toLocaleUpperCase(), canvas.width / 2.9 - textWidth / 2.9, canvas.height * 0.95);
